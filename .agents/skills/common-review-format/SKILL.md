---
name: common-review-format
description: >-
  コードレビューの指摘コメントフォーマットを定義する共有ナレッジ。
  git-review-pr, ios-review-code など全レビュー系スキルから参照される。
  "レビューフォーマット", "review format", "指摘テンプレート" 等で使用。
---
...
GitHub PR コメントとして投稿するレビュー指摘の統一フォーマット。
全レビュー系スキル（`git-review-pr`, `ios-review-code`）はこのフォーマットに従って出力する。
